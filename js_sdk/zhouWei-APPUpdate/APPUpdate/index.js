...
let isShow = false//判断是否显示 弹窗
/**** 结束 *****/

const platform = uni.getSystemInfoSync().platform;
// 主颜色
const $mainColor = "E23D3C";

// 弹窗图标url 只能用在线链接
const $iconUrl = "/static/update.png";
// 当前本地包版本号
let currentVer = "";
...
// 发起ajax请求获取服务端版本号
const getServerNo = function(version, callback) {
	let httpData = {
	...
	uni.getSystemInfo({
		success: (res) => {
			...
			// 可以用自己项目的请求方法
			baseApi.getversion({system_type:type}).then(res => {
...
const getDownload = function(data) {
	...
	dtask = plus.downloader.createDownload(data.wgt, {
...
export default function() {
	getCurrentNo(versionInfo => {
		...
		getServerNo(versionInfo, res => {
			// console.log(res)
			if(res.data.version_list && res.data.version_list.includes(versionInfo.versionName)){//安卓提审版本判断
				return;
			}
			if (res.data.force_update == 1) { // 强制更新
				if (versionInfo.versionName != res.data.version_id){
					if (platform == "android" && res.data.download_page && res.data.download_page.includes('apk')) {
						store.commit("setdownApk",res.data.download_page)
						return;
					}
					if (platform == "ios" && res.data.download_page) {//判断存在直接更新ipa
						plus.runtime.openURL(res.data.download_page);
						return;
					}
					if (/\.wgt$/i.test(res.data.wgt)) {
						getDownload(res.data);
					} else if (/\.html$/i.test(res.data.wgt)) {
						plus.runtime.openURL(res.data.wgt);
					} else {
						if (platform == "android") {
							getDownload(res.data);
						} else {
				...
				if (versionInfo.versionName != res.data.version_id) {
					if(isShow)return;
					isShow = true;
					updatePopup(res.data, function() {
						if (platform == "android" && res.data.download_page  && res.data.download_page.includes('apk')) {//判断存在直接更新apk
							store.commit("setdownApk",res.data.download_page)
							return;
						}
						if (platform == "ios" && res.data.download_page) {//判断存在直接更新ipa
